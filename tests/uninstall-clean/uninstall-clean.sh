...
[ -s ~/.local/state/openjawz/installed-files ] && ok "A6 installed-files manifest ($(wc -l < ~/.local/state/openjawz/installed-files) paths)" || bad "A6 no installed-files manifest"
...
left="$(find ~ /usr/share/openjawz /var/lib/pacman/sync -newer "$marker" -not -type d -not -path "$HOME/.cache/*" -not -path "$HOME/.gnupg/*" -not -name '.bash_history' 2>/dev/null)"

...
| vm | the daemon *is* the VM's agent; idle-exits after 1800 s with no clients and no sessions | remote | park; after idle-exit it stays down — `SYNAPS_DAEMON_AUTOSPAWN=0`, so a client attach does **not** re-spawn it (and idle-exit is exit 0, which `Restart=on-failure` ignores); restart it explicitly or on next login | headless; `chronos` hook only; linger on |

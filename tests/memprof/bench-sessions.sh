...
run_once() { # N i -> prints "pss_kb rss_anon_kb_of_first_engine procs startup_ms"
  local n=$1 i=$2; local out=/tmp/memprof-$TAG-N$n-r$i.txt
...
run_once_daemon() { # N i -> prints "pss_kb daemon_pss_kb procs_beyond_daemon startup_ms out"
  local n=$1 i=$2; local out=/tmp/memprof-$TAG-daemon-N$n-r$i.txt
  tmux -L bench kill-server 2>/dev/null; sleep 1
...
run_once_parked() { # N i -> prints "live_anon_kb parked_anon_kb dprocs_live dprocs_parked attach_ms out"
  local n=$1 i=$2; local out=/tmp/memprof-$TAG-parked-N$n-r$i.txt
  tmux -L bench kill-server 2>/dev/null; sleep 1
  "$BIN" daemon stop >/dev/null 2>&1
  local ids=() s
  for s in $(seq 1 "$n"); do
...
run_once_client() { # N i -> prints "post_med_kb pre_med_kb threads first_frame_ms attach_ms danon out"
  local n=$1 i=$2; local out=/tmp/memprof-$TAG-client-N$n-f${FIXTURE_MSGS_MB}-r$i.txt
  tmux -L bench kill-server 2>/dev/null; sleep 1
  "$BIN" daemon stop >/dev/null 2>&1
  local ids=() s
  for s in $(seq 1 "$n"); do

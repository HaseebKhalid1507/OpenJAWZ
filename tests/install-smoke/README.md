...
docker run -d --name oj-smoke \
  ...
  -v $work:/work -v $work/oj-smoke.service:/etc/systemd/system/oj-smoke.service:ro \
...
and then **no `docker exec`, ever**: the test is driven by `oj-smoke.service` (a oneshot unit file bind-mounted read-only at its unit path and its `.wants` path) which runs `driver.sh`, and the host only tails `$work/driver.log` and waits for `$work/result`.
...
Host-side check after starting (both must hold): `journalctl -b --since "30 sec ago" | grep "Couldn't move" | grep -c docker-` → `0`, and `loginctl list-sessions` unchanged.

...
| tier | when | tests |
|---|---|---|
| `fast` | every push, < 10 s, no container | `grep-guard`, `lint`, `templates`, `tools`, `brain`, `hook-filter`, `pkg`, `migrate`, `boot-db-sig`, `build-repo-pkgrel`, `ambient-unit`, `memprof` (dry parse only) |

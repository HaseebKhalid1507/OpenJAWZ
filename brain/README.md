...
Memory. One `.r8` per user, every session reads. Two axel processes (MCP server for tool calls, extension for
session-end extraction) share it — "one writer" = one brain file, no second engine (`docs/brain.md`).
...
| `secret-patterns.txt` | → `tests/grep-guard/secrets.txt` — one list for the repo guard, the extractor role and `openjawz brain scan` |
...
| `axel-consolidate.{service,timer}` | every 6 h: reindex → strengthen → reorganize → prune → `brain scan` (deletes + logs secret-shaped memories) |
| `last-consolidation.sh` | one sqlite line: when it last ran and what it did |
| `init.sh` | → `openjawz brain init` |

Verb: `openjawz brain init | status | backup [--quick] | consolidate | scan | purge [--yes]` (`bin/openjawz-brain`).
